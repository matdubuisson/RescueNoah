...
class BirthdayMap {
    ...
    void addPerson(Person person) {
        // TODO
        String[] elements = person.birthday.split("-");
        String year = elements[0];

        if (!this.map.containsKey(year)) {
            ArrayList<Person> array = new ArrayList<Person>();
            array.add(person);
            this.map.put(year, array);
        } else {
            this.map.get(year).add(person);
        }

        if (!this.map.containsKey(person.birthday)) {
            ArrayList<Person> array = new ArrayList<Person>();
            array.add(person);
            this.map.put(person.birthday, array);
        } else {
    ...
    List<Person> getPeopleBornInYear(String year) {
        return this.getPeopleBornOnDate(year);
    }
