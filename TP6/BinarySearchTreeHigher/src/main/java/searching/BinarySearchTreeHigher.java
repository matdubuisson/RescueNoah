...
public class BinarySearchTreeHigher<Key extends Comparable<Key>, Value> {
    ...
    public Key minKey() {
         ...
         else return this.minKey(this.root);
    }

    public Key minKey(Node root){
        if(root.left == null) return root.key;
        else return this.minKey(root.left);
    }
    ...
    public Key higherKey(Key key) {
         if(this.root == null) return null;
         else return this.higherKey(this.root, key, null);
    }

    private Key higherKey(Node x, Key key, Key bestKey) {
         if(x == null) return bestKey;

         int cmp = key.compareTo(x.key);

         if(cmp >= 0) return this.higherKey(x.right, key, bestKey);
         else {
             if(bestKey == null || x.key.compareTo(bestKey) < 0) bestKey = x.key;

             return this.higherKey(x.left, key, bestKey);
         }
    }
