...
public class PreorderToBST {
    ...
    */
    ...
    public Node preorderRead(int[] preOrderInput, int i, int min, int max) {
        // !!!!!!! TODO !!!!!!!!
        if(i >= preOrderInput.length || preOrderInput[i] <= min || preOrderInput[i] >= max) return null;

        Node left = preorderRead(preOrderInput, i + 1, min, preOrderInput[i]);
        return new Node(left, preorderRead(preOrderInput, i + 1 + (left == null ? 0 : left.size), preOrderInput[i], max), preOrderInput[i]);
    }
