...
public class ArrayBST<Key extends Comparable<Key>, Value> {
    ...
    public boolean put(Key key, Value val) {
        int parentNodeId = this.NONE, curNodeId = 0, cmp, size = this.keys.size();

        while(curNodeId < size && curNodeId != this.NONE){
            cmp = key.compareTo(this.keys.get(curNodeId));
            parentNodeId = curNodeId;

            if(cmp == 0){
                this.values.set(curNodeId, val);
                return false;
            } else if(cmp < 0) curNodeId = this.idxLeftNode.get(curNodeId);
            else curNodeId = this.idxRightNode.get(curNodeId);

        }

        // tree is empty or fall on leaf (curNodeId == 0 or NONE)
        this.keys.add(key);
        this.values.add(val);
        this.idxLeftNode.add(this.NONE);
        this.idxRightNode.add(this.NONE);

        if(parentNodeId != this.NONE){
            cmp = key.compareTo(this.keys.get(parentNodeId));

            if(cmp < 0) this.idxLeftNode.set(parentNodeId, size);
            else this.idxRightNode.set(parentNodeId, size);
        }
    ...
    public Value get(Key key) {
        int curNodeId = 0, cmp, size = this.keys.size();

        while(curNodeId < size && curNodeId != this.NONE){
            cmp = key.compareTo(this.keys.get(curNodeId));

            if(cmp == 0) return this.values.get(curNodeId);
            else if(cmp < 0) curNodeId = this.idxLeftNode.get(curNodeId);
            else curNodeId = this.idxRightNode.get(curNodeId);
        }
