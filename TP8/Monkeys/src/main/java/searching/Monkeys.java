...
public class Monkeys {
    ...
    public static long evaluate(Monkey monkey, Hashtable<String, Monkey> map){
        if(monkey instanceof YellingMonkey) return ((YellingMonkey) monkey).number;
        else if(monkey instanceof OperationMonkey){
            OperationMonkey m = (OperationMonkey) monkey;
            switch (m.op){
                case '+': {
                    return evaluate(map.get(m.leftMonkey), map) + evaluate(map.get(m.rightMonkey), map);
                } case '-': {
                    return evaluate(map.get(m.leftMonkey), map) - evaluate(map.get(m.rightMonkey), map);
                } case '*': {
                    return evaluate(map.get(m.leftMonkey), map) * evaluate(map.get(m.rightMonkey), map);
    ...
    public static long evaluateRoot(List<Monkey> input) {
        Hashtable<String, Monkey> map = new Hashtable<>();

        for(Monkey monkey : input){
            map.put(monkey.name, monkey);
        }

        return evaluate(map.get("root"), map);
    }
