...
public class RedBlackTreeConverter {
    ...
    public static <Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
        ...
        if(twoThreeNode.isLeaf()){
            return new RBNode<Key>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Black, twoThreeNodeSize(twoThreeNode));
        } else if(twoThreeNode.is2node()){
            RBNode<Key> newNode = new RBNode<Key>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Black, twoThreeNodeSize(twoThreeNode));
            newNode.leftChild = convert(twoThreeNode.leftChild);
            newNode.rightChild = convert(twoThreeNode.centerChild);
            return newNode;
        } else {
            int size = 1 + twoThreeNodeSize(twoThreeNode.leftChild) + twoThreeNodeSize(twoThreeNode.centerChild);
            RBNode<Key> newNode = new RBNode<Key>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red, size);
            newNode.leftChild = convert(twoThreeNode.leftChild);
            newNode.rightChild = convert(twoThreeNode.centerChild);
            RBNode<Key> newParentNode = new RBNode<Key>(twoThreeNode.rightKey, twoThreeNode.rightValue, Color.Black, 1 + size + twoThreeNodeSize(twoThreeNode.rightChild));
            newParentNode.leftChild = newNode;
            newParentNode.rightChild = convert(twoThreeNode.rightChild);
    ...
    public static <Key extends Comparable<Key>> int twoThreeNodeSize(TwoThreeNode<Key> twoThreeNode){
        if(twoThreeNode == null) return 0;
        else if(twoThreeNode.is2node()) return 1 + twoThreeNodeSize(twoThreeNode.leftChild) + twoThreeNodeSize(twoThreeNode.centerChild);
        else return 1 + twoThreeNodeSize(twoThreeNode.leftChild) + twoThreeNodeSize(twoThreeNode.centerChild) + twoThreeNodeSize(twoThreeNode.rightChild);
    }
