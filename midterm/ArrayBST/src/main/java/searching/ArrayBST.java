...
public class ArrayBST<Key extends Comparable<Key>, Value> {
    ...
    public boolean put(Key key, Value val) {
        int cursor = 0, parentCursor = -1, cmp = 0;

        while(cursor != -1 && cursor < this.keys.size()){
            parentCursor = cursor;
            cmp = key.compareTo(this.keys.get(cursor));

            if(cmp == 0){
                this.values.set(cursor, val);
                return false;
            } else if(cmp < 0) cursor = this.idxLeftNode.get(cursor);
            else cursor = this.idxRightNode.get(cursor);
        }

        if(parentCursor != -1){
            if(cmp < 0) this.idxLeftNode.set(parentCursor, this.keys.size());
            else this.idxRightNode.set(parentCursor, this.keys.size());
        }

        this.keys.add(key);
        this.values.add(val);
        this.idxLeftNode.add(-1);
        this.idxRightNode.add(-1);
    ...
    public Value get(Key key) {
        if(this.keys.size() == 0) return null;
         ...
         while(cursor != -1 && cursor < this.keys.size()){
             ...
             if(cmp == 0) return this.values.get(cursor);
             else if(cmp < 0) cursor = this.idxLeftNode.get(cursor);
             else cursor = this.idxRightNode.get(cursor);
         }
