...
public class BinarySearchTreeIterator<Key extends Comparable<Key>> implements Iterable<Key> {
    ...
    private class BSTIterator implements Iterator<Key> {
        // FAIL-FAST
        int nOp = BinarySearchTreeIterator.this.nOp;
        BSTNode<Key> root = BinarySearchTreeIterator.this.root;
        int top = -1;
        BSTNode<Key>[] stack = new BSTNode[BinarySearchTreeIterator.this.size()];


        @Override
        public boolean hasNext() {
            if(this.nOp != BinarySearchTreeIterator.this.nOp) throw new ConcurrentModificationException();
            else return this.top != -1 || this.root != null;
        }

        @Override
        public Key next() {
            if(!this.hasNext()) throw new NoSuchElementException();
            else if(this.nOp != BinarySearchTreeIterator.this.nOp) throw new ConcurrentModificationException();
            else {
        ...
        /*
        int index = 0, nOp = BinarySearchTreeIterator.this.nOp;
        Key[] array;

        public BSTIterator(){
            int index = 0;
            this.array = (Key[]) new Comparable[BinarySearchTreeIterator.this.size()];

            BSTNode<Key> root = BinarySearchTreeIterator.this.root;
            int top = -1;
            BSTNode<Key>[] stack = new BSTNode[this.array.length];
        ...
        public boolean hasNext() {
            if(this.nOp != BinarySearchTreeIterator.this.nOp) throw new ConcurrentModificationException();
            else return this.index < this.array.length;
        }

        @Override
        public Key next() {
            if(this.index >= this.array.length) throw new NoSuchElementException();
            else if(this.nOp != BinarySearchTreeIterator.this.nOp) throw new ConcurrentModificationException();
