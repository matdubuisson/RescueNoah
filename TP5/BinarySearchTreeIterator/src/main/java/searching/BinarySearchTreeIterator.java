...
public class BinarySearchTreeIterator<Key extends Comparable<Key>> implements Iterable<Key> {
    ...
    private class BSTIterator implements Iterator<Key> {
        private BSTNode<Key> root = BinarySearchTreeIterator.this.root;
        private int nOp = BinarySearchTreeIterator.this.nOp, index = 0;

        private ArrayList<Key> array;

        public BSTIterator(){
            this.array = new ArrayList<Key>();

            this.makeArray(this.root);
        }

        private void makeArray(BSTNode<Key> root){
            if(root == null) return;

            if(root.left != null){
                this.makeArray(root.left);
            }

            this.array.add(root.key);

            if(root.right != null){
                this.makeArray(root.right);
            }
        }
        ...
        @Override
        public Key next() {
            if(this.index >= this.array.size()) throw new NoSuchElementException();
            if(this.nOp != BinarySearchTreeIterator.this.nOp) throw new ConcurrentModificationException();
