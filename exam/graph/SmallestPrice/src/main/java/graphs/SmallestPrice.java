...
public class SmallestPrice {
    ...
    private static class MyPair implements Comparable<MyPair>{
        ...
        public MyPair(int source, int distance){
            this.source = source;
    ...
    public static int getSmallestPrice(WeightedGraph graph, int source, int maxTime, List<Pair> destinations) {
        ...
        PQ.add(new MyPair(source, 0));

        while(!PQ.isEmpty()){
            MyPair p = PQ.poll();

            for(DirectedEdge e : graph.outEdges(p.source)){
                if(distances[e.to()] > distances[e.from()] + e.weight() && distances[e.from()] + e.weight() <= maxTime){
                    distances[e.to()] = distances[e.from()] + e.weight();
                    PQ.add(new MyPair(e.to(), distances[e.to()]));
                }
            }
        }
        ...
        for(Pair destination : destinations){
            if(distances[destination.node] != Integer.MAX_VALUE){
                if(bestPrice > destination.price) bestPrice = destination.price;
            }
        }

        return bestPrice == Integer.MAX_VALUE ? -1 : bestPrice;
    }
