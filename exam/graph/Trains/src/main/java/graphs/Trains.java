...
public class Trains {
    ...
    private static class Pair implements Comparable<Pair>{
        ...
        public Pair(StationTime stationTime, int distance){
    ...
    private static Iterable<StationTime> adj(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint){
        LinkedList<StationTime> ws = new LinkedList<StationTime>();

        for(StationTime key : relations.keySet()){
            if(key.station.equals(startPoint.station) && startPoint.time <= key.time){
                for(StationTime w : relations.get(key)){
                    ws.add(w);
                }
            }
        }
    ...
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        HashMap<String, Integer> distances = new HashMap<String, Integer>(); // null means INFINITY
        distances.put(startPoint.station, startPoint.time);

        PriorityQueue<Pair> PQ = new PriorityQueue<Pair>();
        PQ.add(new Pair(startPoint, startPoint.time));

        while(!PQ.isEmpty()){
            Pair v = PQ.poll();
            //System.out.println("==================");
            //System.out.println(v.stationTime.station);

            for(StationTime w : adj(relations, v.stationTime)){
                //System.out.println(w.station);
                Integer actualDistance = distances.get(w.station);
                Integer newDistance = w.time;

                if(actualDistance == null || actualDistance > newDistance){
                    distances.put(w.station, newDistance);
                    PQ.add(new Pair(w, newDistance));
                }
            }
        }
